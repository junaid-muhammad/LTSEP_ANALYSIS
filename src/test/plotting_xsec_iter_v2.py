...
# sigma_L fit
# All parameters are free and can be negative. Use wide bounds to improve fit stability.
p0_L = [p5, p6, p7, p8, p9]  # Initial guess from file
lower_bounds = [-1e6, -1e4, -1e4, -1e4, -1e4]
upper_bounds = [1e6, 1e4, 1e4, 1e4, 1e4]
try:
    ...
    popt_L, pcov_L = curve_fit(
        ...
        bounds=(lower_bounds, upper_bounds), absolute_sigma=True, maxfev=30000
    )
...
# sigma_T fit
p0_T = [p1, p2]
lower_bounds_T = [-1e6, -1e6]
upper_bounds_T = [1e6, 1e6]
try:
    ...
    popt_T, pcov_T = curve_fit(
        ...
        bounds=(lower_bounds_T, upper_bounds_T), absolute_sigma=True, maxfev=30000
    )
...
lower_bounds_LT = [-1e6, -1e6, -1e6, -1e6]
upper_bounds_LT = [1e6, 1e6, 1e6, 1e6]
try:
    ...
    popt_LT, pcov_LT = curve_fit(
        ...
        bounds=(lower_bounds_LT, upper_bounds_LT), absolute_sigma=True, maxfev=30000
    )
...
try:
    ...
    popt_TT, pcov_TT = curve_fit(
        ...
        bounds=(lower_bounds_TT, upper_bounds_TT), absolute_sigma=True, maxfev=30000
    )
