...
theta_cm_var = sep_xsection_df['theta_cm'].values
m_pi = 0.13957
...
def sigma_T_func(Q2_var, A, B):
    return A / Q2_var + B / Q2_var**2
# C and D are unassigned parameters in the code so far.

def sigma_L_func(abs_t_cent_var, Q2_var, E, F, G, H, I):
    return E * (Q2_var / (1 + H * Q2_var + I * Q2_var**2)**2) * np.exp((F - G * np.log(Q2_var)) * np.abs(abs_t_cent_var))

def sigma_LT_func(abs_t_cent_var, Q2_var, J, K, L, M, theta_cm_var):
    return (np.exp(J + K / np.sqrt(Q2_var) * np.abs(abs_t_cent_var)) + L + (M / Q2_var**2)) * np.sin(theta_cm_var)

def sigma_TT_func(abs_t_cent_var, Q2_var, N, m_pi, theta_cm_var):
    return (N / Q2_var**2) * ((np.abs(abs_t_cent_var)) / ((np.abs(abs_t_cent_var)) + m_pi**2)**2) * np.sin(theta_cm_var)**2
...
A = params[0]  #1
B = params[1]  #2
C = params[2]  #3 - unassigned
D = params[3]  #4 - unassigned
E = params[4]  #5
F = params[5]  #6
G = params[6]  #7
H = params[7]  #8
...
M = params[12] #13
N = params[13] #14
O = params[14] #15 - unassigned
P = params[15] #16 - unassigned
print ("Fit parameters loaded from file:")
print(f"A={A}, B={B}, C={C}, D={D}, E={E}, F={F}, G={G}, H={H}, I={I}, J={J}, K={K}, L={L}, M={M}, N={N}, O={O}, P={P}")

# --- Fit functional forms to data and print new parameters ---
# sigma_L fit
p0_L = [E, F, G, H, I]  # initial guess from loaded params
try:
    def sigma_L_fitfunc(x_tuple, E, F, G, H, I):
        t, Q2 = x_tuple
        return sigma_L_func(t, Q2, E, F, G, H, I)
    x_input_L = np.vstack((abs_t_cent_var, Q2_var))
    popt_L, pcov_L = curve_fit(sigma_L_fitfunc, x_input_L, sigma_L, sigma=sigma_L_err, p0=p0_L, absolute_sigma=True, maxfev=10000)
    print("New sigma_L fit params: E, F, G, H, I =", popt_L)
except Exception as e:
    print("sigma_L fit failed:", e)
    popt_L = p0_L
    pcov_L = np.zeros((len(p0_L), len(p0_L)))

# sigma_T fit
p0_T = [A, B]
try:
    def sigma_T_fitfunc(Q2, A, B):
        return sigma_T_func(Q2, A, B)
    popt_T, pcov_T = curve_fit(sigma_T_fitfunc, Q2_var, sigma_T, sigma=sigma_T_err, p0=p0_T, absolute_sigma=True, maxfev=10000)
    print("New sigma_T fit params: A, B =", popt_T)
except Exception as e:
...
p0_LT = [J, K, L, M]
try:
    def sigma_LT_fitfunc(x_tuple, J, K, L, M):
        t, Q2, theta_cm = x_tuple
        return sigma_LT_func(t, Q2, J, K, L, M, theta_cm)
    x_input_LT = np.vstack((abs_t_cent_var, Q2_var, theta_cm_var))
    popt_LT, pcov_LT = curve_fit(sigma_LT_fitfunc, x_input_LT, sigma_LT, sigma=sigma_LT_err, p0=p0_LT, absolute_sigma=True, maxfev=10000)
    print("New sigma_LT fit params: J, K, L, M =", popt_LT)
except Exception as e:
...
p0_TT = [N]
try:
    def sigma_TT_fitfunc(x_tuple, N):
        t, Q2, theta_cm = x_tuple
        return sigma_TT_func(t, Q2, N, m_pi, theta_cm)
    x_input_TT = np.vstack((abs_t_cent_var, Q2_var, theta_cm_var))
    popt_TT, pcov_TT = curve_fit(
        sigma_TT_fitfunc, x_input_TT, sigma_TT, sigma=sigma_TT_err, p0=p0_TT, absolute_sigma=True, maxfev=10000
    )
    print("New sigma_TT fit param: N =", popt_TT)
except Exception as e:
...
# σ_L vs t
axs4[0].errorbar(abs_t_cent_var, sigma_L, yerr=sigma_L_err, fmt='o', color='blue', label='Data')
...
axs4[0].plot(abs_t_cent_var, sigma_L_fit, 'r--', label='Fit')
axs4[0].legend()

# σ_T vs t
axs4[1].errorbar(abs_t_cent_var, sigma_T, yerr=sigma_T_err, fmt='s', color='red', label='Data')
...
axs4[1].plot(abs_t_cent_var, sigma_T_fit, 'r--', label='Fit')
axs4[1].legend()

# σ_LT vs t
axs4[2].errorbar(abs_t_cent_var, sigma_LT, yerr=sigma_LT_err, fmt='^', color='green', label='Data')
...
axs4[2].plot(abs_t_cent_var, sigma_LT_fit, 'r--', label='Fit')
axs4[2].legend()

# σ_TT vs t
axs4[3].errorbar(abs_t_cent_var, sigma_TT, yerr=sigma_TT_err, fmt='v', color='purple', label='Data')
...
axs4[3].plot(abs_t_cent_var, sigma_TT_fit, 'r--', label='Fit')
axs4[3].legend()
...
# Sort alphabetically
param_tuples = sorted(zip(param_names, param_values, param_errors), key=lambda x: x[0])
# Write to file
param_outfile = f"{XSECT_OUTPATH}/new_fitparams_iter{ITERATION}_par.pl_{Q2par}"
with open(param_outfile, 'w') as f:
    for idx, (name, val, err) in enumerate(param_tuples, 1):
