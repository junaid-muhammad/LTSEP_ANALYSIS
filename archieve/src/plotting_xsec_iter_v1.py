...
# Grab Q2, t_central, and theta_cm columns as arrays
...
def sigma_L_func(abs_t_cent_var, Q2_var, E, F, G, H, I):
    return E * Q2_var / (1 + F * Q2_var + G * Q2_var**2)**2 * np.exp((H - I * np.log(Q2_var)) * (abs_t_cent_var))

def sigma_LT_func(abs_t_cent_var, Q2_var, J, K, L, M, theta_cm_var):
    return (np.exp(J + K / np.sqrt(Q2_var) * (abs_t_cent_var)) + L + M / Q2_var**2) * np.sin(theta_cm_var)

def sigma_TT_func(abs_t_cent_var, Q2_var, N, m_pi, theta_cm_var):
    return N / Q2_var**2 + (abs_t_cent_var) / ((abs_t_cent_var) + m_pi**2)**2 * np.sin(theta_cm_var)**2
